...
# ─────────────────────────────────────────
# ONBOARDING — runs once if plates.json doesn't exist
# ─────────────────────────────────────────
if not plates_exist():
    st.title("Welcome to BookePaid 💜💙")
    st.subheader("Let's set up your menu")
    st.write("Tell us what kind of business you run and we'll generate a starter set of menu items with typical ingredients automatically.")
    ...
    if st.button("Generate My Menu Templates ✨"):
        with st.spinner(f"Generating menu items for {restaurant_type}..."):
            try:
                ...
                save_plates(plates)
                st.success(f"✅ Generated {len(plates)} menu items for your {restaurant_type}!")
                st.rerun()
...
# ─────────────────────────────────────────
    ...
    # Reset onboarding
    if st.button("🔄 Reset Menu Templates"):
        import os
        if os.path.exists("plates.json"):
            os.remove("plates.json")
        st.success("Menu templates reset. Refresh to run onboarding again.")
        st.rerun()
