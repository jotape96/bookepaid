...
PROMPT = """Extract all line items from this invoice and return ONLY a JSON array.
...
- invoice_number (string): invoice number or reference, "UNKNOWN" if not found
...
def extract_invoice_image(image_bytes: bytes, image_type: str, invoice_name: str) -> pd.DataFrame:
    """Send image invoice to Claude, extract line items, return as DataFrame."""
    client = get_client()
